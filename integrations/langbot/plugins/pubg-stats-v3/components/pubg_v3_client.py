...
LOCAL_PUBG_SIGNAL = re.compile(
    r'PUBG|绝地求生|吃鸡|战绩|KD|K/D|击杀|杀人|人头|伤害|助攻|倒地|击倒|救援|扶人|排名|名次|场均|几把|多少场|谁最强|谁最菜|谁最拉|拉完了|发挥最好|状态最好|表现最好|复盘|对局复盘|分析某一局|分析这把|分析这局|看看这把怎么打|看看这局怎么打|最后一把为什么输|这把怎么打|上一把|下一把|火箭筒|重武器|开车|载具|最近\s*\d+\s*(?:场|把|局)|最近\s*\d+\s*天|上周|前天|昨天|昨日|昨晚|今天|今日|\b[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}\b',
    re.IGNORECASE,
)
