...
function addPlayer(accumulator: Accumulator, match: NormalizedMatch, player: NormalizedPlayer): void {
  ...
  else accumulator.unknownDeathCount += 1;
...
function metricValue(accumulator: Accumulator, metric: Metric): number | string | null {
  ...
  const kd = accumulator.unknownDeathCount > 0 ? null : accumulator.deaths > 0 ? accumulator.kills / accumulator.deaths : null;
  switch (metric) {
    ...
    case 'kd': return kd;
    case 'deaths': return accumulator.unknownDeathCount > 0 ? null : accumulator.deaths;
...
function createRow(key: string, label: string, groupBy: GroupBy, accumulator: Accumulator, metadata: Partial<QueryRow> = {}): QueryRow {
  ...
  metrics.denominatorZero = accumulator.matchIds.size === 0 || accumulator.unknownDeathCount > 0 ? null : accumulator.deaths === 0 ? 1 : 0;
  return {
    key,
    label,
    groupBy,
...
function aggregateTeamRow(records: NormalizedMatch[], ids: Set<string>, team: TeamConfig): QueryRow {
  ...
  row.metrics.teamCombinedKD = accumulator.unknownDeathCount > 0 || accumulator.deaths === 0 ? null : round(accumulator.kills / accumulator.deaths);
...
function aggregateCategoryRows(records: NormalizedMatch[], ids: Set<string>, team: TeamConfig, groupBy: 'map' | 'mode'): QueryRow[] {
  const groups = new Map<string, NormalizedMatch[]>();
  for (const record of records) {
    const label = groupBy === 'map' ? record.mapName : record.gameMode;
    const bucket = groups.get(label) ?? [];
    bucket.push(record);
    groups.set(label, bucket);
  }
  return [...groups.entries()].sort(([left], [right]) => left < right ? -1 : left > right ? 1 : 0).map(([label, matches]) => {
    const row = aggregateTeamRow(matches, ids, team);
    return { ...row, key: label, label, groupBy };
  });
}
...
export class DeterministicQueryEngine {
  ...
  private executeSingle(
    ...
    else if (query.groupBy === 'map' || query.groupBy === 'mode') rows = aggregateCategoryRows(selected, subjectIds, team, query.groupBy);
    else rows = [aggregateTeamRow(selected, subjectIds, team)];
    if (query.groupBy !== 'match') {
    ...
    return {
      ...
      data: {
        ...
        summary: {
          ...
          comparisonRatios: Object.fromEntries(deltaRows.map((row) => [row.key, row.comparisonRatios ?? {}])),
        },
