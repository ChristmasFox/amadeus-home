...
const PUBG_INTENT_SIGNAL = /PUBG|绝地求生|吃鸡|战绩|战报|KD|K\/D|击杀|杀人|人头|伤害|助攻|倒地|击倒|救援|扶人|复盘|分析(?:这把|这局|这场|某一局|某一场|战绩|表现|数据)|火箭筒|排名|名次|场均|几把|多少场|最近\s*\d+\s*(?:场|把|局)|谁最强|谁最菜|谁最拉|拉完了|发挥最好|状态最好|表现最好|整活|离谱|内鬼|打队友|撞人|闪光弹|拳击|队伤|队友伤害|乘车|旅游团|有什么节目/u;
...
const TIME_RANGE_FOLLOW_UP = /^(?:(?:昨天|昨日|前天|大前天|今天|今日|昨晚|昨天晚上|昨日晚上|(?:\d{4}\s*年\s*)?\d{1,2}\s*月\s*\d{1,2}\s*(?:日|号)?|\d{4}\s*[-/]\s*\d{1,2}\s*[-/]\s*\d{1,2})(?:\s*(?:呢|吗|吧|如何|怎么样|战绩|表现|数据))?|晚上)(?:[？?！!。．.\s]*)$/u;
const PUBG_REFERENCE_FOLLOW_UP = /^(?:刚才|那|这|其中|他|他们|上一场|那场|哪一把|哪把|谁|跟|和|比较|对比|怎么样|如何|呢|多少|几把|最强|最菜|最拉|拉完了|整活|离谱|内鬼|打队友|撞人|闪光弹|拳击|队伤|队友伤害|乘车|旅游团|节目)(?:.{0,48})$/u;
...
export function classifyPubgRequest(text: string, context: SessionContextRecord | null): DomainRouteResult {
  ...
  if (contextActive && (TIME_RANGE_FOLLOW_UP.test(normalized) || PUBG_REFERENCE_FOLLOW_UP.test(normalized))) {
