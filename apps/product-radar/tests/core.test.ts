...
test('notification delivery is idempotent and one failed channel does not block the owner channel', async () => {
  ...
  const owner = new FakeChannel('owner-notification', 'owner');
...
test('heartbeat digest is due once, includes no-match status, and keeps channels independent', async () => {
  ...
  assert.equal(owner.calls.length, 1);
  const payload = owner.calls[0]!.payload as Record<string, unknown>;
  assert.equal(payload.kind, 'heartbeat');
  assert.equal(payload.imageModelCalls, 1);
  assert.equal(payload.imageModelImagesProcessed, 1);
