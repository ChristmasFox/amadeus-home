...
这些模板覆盖迁移时最容易遗漏的 aria2、9router、Filebrowser、Xiaoya 和 Jellyfin。它们只保存
路径、端口、镜像和变量名；RPC secret、JWT secret、初始密码和 machine salt
必须由外部 secret store 注入。

9router、Filebrowser 与 Xiaoya 的迁移模板默认只绑定 guest loopback，供本机验收，
...
Alist storage 配置后才能宣称 Avalon-backed 功能已恢复。
