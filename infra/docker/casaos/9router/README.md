...
Bridge caps: 7 MiB multipart input, 6 MiB converted mono 16 kHz WAV (about three minutes), 80 s upstream timeout, 30 s conversion timeout, no retry, max 128 successful transcript entries held in memory for five minutes, with same-content in-flight coalescing and at most eight concurrent unique requests. Logs contain only a 12-hex request hash, size/duration buckets, cache outcome and elapsed time; no private audio, transcript text, tokens or full provider errors are logged. A TTS service outage does not stop this independent ASR process. The 9Router container has a supervisor for the bridge and packaged server; if either exits the container restarts under CasaOS. No secret asset or production voice source is in the image.
