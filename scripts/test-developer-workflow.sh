...
mlx_engine_source="$(scope infra/macos/qwen3-tts-engine.json infra/macos/requirements-mlx-tts.txt infra/macos/verify-qwen3-mlx-assets.py)"
assert_line "$mlx_engine_source" 'CHANGE_SCOPE_WORKFLOW=QWEN3_TTS'
b_audit="$(scope scripts/audit-qwen-b-reference.py scripts/test-audit-qwen-b-reference.py scripts/align-qwen-b-reference.py scripts/test-align-qwen-b-reference.py)"
assert_line "$b_audit" 'CHANGE_SCOPE_WORKFLOW=QWEN3_TTS'
...
router_deploy="$(scope scripts/deploy-9router-speech.sh)"
assert_line "$router_deploy" 'CHANGE_SCOPE_WORKFLOW=ROUTER_RELEASE_CONFIG'
router_qwen="$(scope scripts/provision-9router-qwen-asr.sh)"
assert_line "$router_qwen" 'CHANGE_SCOPE_WORKFLOW=ROUTER_RELEASE_CONFIG'
router_fixture="$(scope scripts/test-9router-speech-image.sh)"
assert_line "$router_fixture" 'CHANGE_SCOPE_WORKFLOW=ROUTER_RELEASE_CONFIG'
...
report_fixture="$(scope scripts/check-tts-performance-report.py scripts/export-tts-performance-data.py)"
assert_line "$report_fixture" 'CHANGE_SCOPE_LEVEL=FAST'
assert_line "$report_fixture" 'DOCKER_IMAGE_SET=none'
cache_fixture="$(scope scripts/test-openclaw-docker-cache-order.py)"
assert_line "$cache_fixture" 'CHANGE_SCOPE_LEVEL=FAST'
assert_line "$cache_fixture" 'DOCKER_IMAGE_SET=none'
...
pure_voice_fixture="$(scope scripts/test-openclaw-voice-policy.mjs)"
assert_line "$pure_voice_fixture" 'CHANGE_SCOPE_WORKFLOW=VOICE_PATCH'
