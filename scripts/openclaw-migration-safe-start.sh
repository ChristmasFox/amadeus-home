...
openclaw_state="$(orb -m "$MACHINE" -u root docker inspect --format '{{.State.Status}}' openclaw 2>/dev/null || printf absent)"
[[ "$openclaw_state" != running ]] || { printf '%s\n' 'MIGRATION_SAFE_OPENCLAW=BLOCKED an OpenClaw container is already running.'; exit 1; }
radar_state="$(orb -m "$MACHINE" -u root docker inspect --format '{{.State.Status}}' product-radar 2>/dev/null || printf absent)"
[[ "$radar_state" != running ]] || { printf '%s\n' 'MIGRATION_SAFE_OPENCLAW=BLOCKED Product Radar must remain stopped.'; exit 1; }
ingress="$(orb -m "$MACHINE" -u root docker ps --format '{{.Names}} {{.Image}}' | awk 'tolower($0) ~ /(frpc|nginx-proxy-manager|nginxproxymanager|(^|[ /])npm([ :/]|$))/ {n++} END {print n+0}')"
[[ "$ingress" == 0 ]] || { printf '%s\n' 'MIGRATION_SAFE_OPENCLAW=BLOCKED a destination ingress container is running.'; exit 1; }
orb -m "$MACHINE" -u root test -f "$DATA_ROOT/config/openclaw.json" && orb -m "$MACHINE" -u root test ! -L "$DATA_ROOT/config/openclaw.json" || { printf '%s\n' 'MIGRATION_SAFE_OPENCLAW=BLOCKED canonical restored config is missing or symlinked.'; exit 1; }
...
orb -m "$MACHINE" -u root python3 -c "$remote_code" --source "$DATA_ROOT/config/openclaw.json" --output "$overlay_config" --apply --approve-avalon-move "$APPROVAL"
...
orb -m "$MACHINE" -u root python3 - "$APP_DIR/docker-compose.migration-safe.yml" "$compose_payload" <<'PY'
...
orb -m "$MACHINE" -u root python3 - "$APP_DIR" <<'PY'
...
cd "$APP_DIR"
orb -m "$MACHINE" -u root bash -lc "cd '$APP_DIR' && docker compose -f docker-compose.yml -f docker-compose.migration-safe.yml up -d --no-build"

healthy=0
for _ in {1..30}; do
  status="$(orb -m "$MACHINE" -u root docker inspect --format '{{.State.Health.Status}}' openclaw 2>/dev/null || true)"
...
orb -m "$MACHINE" -u root docker exec openclaw node -e 'const fs=require("fs"); const c=JSON.parse(fs.readFileSync(process.env.OPENCLAW_CONFIG_PATH,"utf8")); if(c.gateway?.bind!=="loopback" || Object.values(c.channels||{}).some(x=>x.enabled!==false) || c.plugins?.entries?.amadeus?.config?.ownerNotificationDeliveryEnabled!==false) process.exit(1);'
orb -m "$MACHINE" -u root docker exec openclaw node -e 'fetch("http://9router:20128/v1/models").then(r=>process.exit((r.status===200||r.status===401)?0:1)).catch(()=>process.exit(1))'
orb -m "$MACHINE" -u root docker exec openclaw node dist/index.js plugins inspect amadeus --runtime --json >/dev/null
