...
| homarr | CasaOS dashboard; live source container mounts Docker socket read-write | `/DATA/AppData/big-bear-homarr/data` | `AUTH_SECRET`; `SECRET_ENCRYPTION_KEY` (not yet covered by a protected Homarr secret bundle entry) | MANUAL_BLOCKER | dashboard data may be rebuilt, but preserve/recreate secrets only through approved secret handling | user must choose Docker-socket access vs. restricted rebuild; do not restore socket by default | local health/auth; verify socket is absent unless explicitly approved | security review |
