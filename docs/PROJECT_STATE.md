...
The first cold-snapshot attempt safely removed its failed partial artifact after detecting Linux guest UID/GID versus macOS verifier ownership mismatch. The verifier fix and a cross-UID/GID regression fixture are implemented and pass focused checks. Final secret bundle verification passed; the cold snapshot and final HomeLab backup remain pending.
