...
- OpenClaw cold state 已恢复并与来源快照校验一致；详细计数、rollback checkpoint、失败恢复副本保留、owner-ingress-off 与未启动状态见 `.agent/checkpoints/2026-09-23-openclaw-destination-restore.md`。旧 Mac 的 `ai.openclaw.gateway` LaunchAgent 仍 active，分类/唯一 runtime gate 未通过前不得启动 M204 OpenClaw。
