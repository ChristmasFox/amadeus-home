...
2026-09-23 Phase 9 OpenClaw cold-state restore passed on M204. The source cold snapshot and encrypted secret bundle were reverified on the destination before apply. Restore matched `MEMORY.md` SHA-256 `9a31ea1e0762065463066600aec74ea56502a95101e488338b29cfc8085bfbd8`, restored 155 workspace files / 20,172,403 bytes, 4,888 state files / 235,858,460 bytes, 84 session/JSONL files and 6 transcripts; Identity and PUBG SQLite both report `ok`. All 859 credential tree entries have numeric owner `1000:1000`; bundle path/content/mode/owner verification passed. Rollback checkpoint `20260923T135605Z-de55825f` retains the pre-restore credentials-only config; earlier failed-new state remains under `20260923T134337Z-b60e28ee/failed-new`, both under root-only backup directories. Destination Docker still contains only the six loopback staging services, process probe reports zero OpenClaw/Gateway processes, and both temporary passphrase staging locations are clean. `DESTINATION_AUTHORITY=NO`; OpenClaw, Product Radar and owner ingress remain off. The old Mac `ai.openclaw.gateway` LaunchAgent is still active (launchd job, port 18789); classify its authority and verify `OPENCLAW_ACTIVE_RUNTIME_COUNT=1` before Phase 12 owner ingress, while Phase 10 migration-safe startup must keep all channels, public ingress, and owner delivery disabled. Evidence: `.agent/checkpoints/2026-09-23-openclaw-destination-restore.md`.

Phase 10 preflight hardening found and fixed a false-green `--plan`: it validates the merged no-ingress Compose and a loaded immutable image. M204 built and loaded `local/openclaw-amadeus:git-9cb5474-20260923145233` (matching host/guest digest `4e7ea2d63ff7a80d98575d60060ad3825567b6fa3fc8e3c860d7b0f8db069412`). The approved apply atomically installed the canonical Compose and migration-safe overlay; merged preflight passed, but startup aborted before Compose because a redundant host-side `cd` targeted a guest-only path. Live inspection confirmed no OpenClaw container was created or started. The redundant `cd` is removed in a follow-up source fix; the canonical Compose and safe overlay remain, and the next apply will re-preflight before starting. No ingress, owner delivery, or production authority is active. Package build/typecheck and 70 package tests, migration-safe/unique-runtime tests, local secrets scan, FAST workflow, syntax, version, and diff checks passed; target `rg` is absent, so its secrets-script run is not accepted as evidence. See `.agent/checkpoints/2026-09-23-migration-safe-compose-preflight.md`.
...
- OpenClaw cold state 已恢复并与来源快照校验一致；详细计数、rollback checkpoint、失败恢复副本保留、owner-ingress-off 与未启动状态见 `.agent/checkpoints/2026-09-23-openclaw-destination-restore.md`。旧 Mac 的 `ai.openclaw.gateway` LaunchAgent 仍 active；Phase 10 migration-safe mode 可隔离验证，分类与唯一 runtime gate 必须在 Phase 12 owner ingress 前通过。
