...
Phase 10 runs on M204 in migration-safe mode: immutable ARM64 image host/guest digest matches, merged preflight passed, container healthy, all channels disabled, no published ports, and owner delivery disabled. The first natural-language Phase 11 probe misrouted aliases to `identity_resolve(reference=self)`, then used `memory_search` on the unavailable configured embedding/provider/index path and an invalid `ls` fallback. A bounded safe-mode probe with local `sessions_search` + `sessions_history` retrieved relevant prior context without embeddings; exact-parameter identity probes resolved the preset alias and correctly returned not-found for the other name. Thus source data exists but ordinary tool routing is defective. The source Skill now requires correct Identity arguments and local session search for historical questions; it is not yet in the candidate image, so operator acceptance remains failed pending a natural-prompt retest. No vector reindex was run because it can send private text to an external embedding provider. Corrected session-continuity counters are fixture-tested; the previous 84/6 path-substring counts are invalid. Separately, classify the old Mac host-local LaunchAgent and recheck the unique-runtime gate before any owner ingress. Evidence: `.agent/checkpoints/2026-09-23-kurisu-recall-audit.md`.
