...
Phase 10 preflight hardening found and fixed a false-green `--plan`: it validates the merged no-ingress Compose and a loaded immutable image. M204 built and loaded `local/openclaw-amadeus:git-9cb5474-20260923145233` (matching host/guest digest `4e7ea2d63ff7a80d98575d60060ad3825567b6fa3fc8e3c860d7b0f8db069412`). Approved apply installed canonical Compose and passed preflight. After a host-path bug was fixed, OpenClaw started but exited 78 because the temporary config overlay added a plugin property rejected by the OpenClaw schema. Read-only `doctor --json` identified the exact key. Source now leaves owner-delivery policy out of plugin JSON and relies on the supported Compose environment override `OWNER_NOTIFICATION_DELIVERY_ENABLED=false`; startup assertion and runtime probe verify that environment. Current container is exited, not healthy; no ingress, owner delivery, or production authority is active. Before retry, preserve the old failed `/run` overlay and regenerate a fresh one. Package build/typecheck and 70 tests, migration-safe/unique-runtime tests, local secrets scan, FAST workflow, syntax, version, and diff checks passed; target lacks `rg`, so its secrets-script output is not accepted as proof. See `.agent/checkpoints/2026-09-23-migration-safe-compose-preflight.md`.
