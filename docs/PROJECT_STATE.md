...
Phase 10 preflight hardening found and fixed a false-green `--plan`: it validates the merged no-ingress Compose and an already loaded immutable `local/openclaw-amadeus:git-<commit>-<timestamp>` image. M204 live `--plan` fail-closed on the absent Compose. The exact-token apply path can atomically create the missing canonical Compose from the Git template only after the immutable image is confirmed loaded; it rejects non-empty app directories, symlinks, or conflicting definitions and still requires merged Compose preflight before startup. M204 currently has no OpenClaw Compose/image and no runtime was started. Target guest has 374 GiB free; target Mac has 393 GiB free; its six service images (3.964 GB) are active, so nothing was pruned. The source/control Mac's 22 GiB free is not the build target. Regression, unique-runtime, FAST workflow, syntax, secrets, version, and diff checks pass. Evidence: `.agent/checkpoints/2026-09-23-migration-safe-compose-preflight.md`.
