...
Phase 10 now runs on M204 in migration-safe mode: immutable ARM64 image host/guest digest matches, merged preflight passed, container healthy, all channels disabled, no published ports, owner delivery disabled, and 9Router probe passed. The runtime probe returned `MIGRATION_SAFE_CANDIDATE=passed` with one M204 candidate. Phase 11 machine acceptance also passed: current MEMORY.md and memory/** hashes, workspace tree/count/bytes match the authenticated manifest (155 files / 20,172,403 bytes); 84 session/JSONL and 6 transcripts readable; all six discovered SQLite databases, including Identity/PUBG, pass integrity; health healthy. Runtime startup added three state files after the cold restore, while all DBs remain intact. Read-only `doctor --json` emitted three onboarding/tool-policy warnings (loopback onboarding, device-pair disabled, skill_workshop sender policy), outside the Goal's listed machine acceptance criteria. Phase 11 operator memory/persona acceptance is pending. Old Mac host-local LaunchAgent authority must be classified and the unique-runtime check freshly repeated before Phase 12 owner ingress. Evidence: `.agent/checkpoints/2026-09-23-migration-safe-compose-preflight.md`.
