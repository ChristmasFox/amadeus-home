...
- Canonical `main` contains the complete 1.5.9 baseline; current short-lived goal branch is `work/post-voice-engineering-performance`. Git release target `VERSION=1.6.0` is staged, not yet formally applied. External references, samples, weights and secrets never enter Git.
...
## Live candidates and rollback

- M204 OrbStack `nyannyan` CasaOS: OpenClaw candidate `local/openclaw-amadeus:git-c75619421d23-20260926104238`, Product Radar `local/product-radar:git-d988000e1c5d-20260924130631`, both healthy, OpenClaw restarts 0, WhatsApp linked. Protected OpenClaw checkpoint `/DATA/AppData/openclaw/backups/amadeus-openclaw-20260926104238`.
...
## Measured limits and pending release
