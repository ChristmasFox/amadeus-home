...
- Isolated MLX 1.7B 8-bit PoC source `356901a` with pinned third-party backend/model completed 12 A/D comparison configs; direct MLX is faster, but peak memory/startup/dependency complexity higher and owner reports D short reference loses Kurisu voice character, so D is not a production option. C/E and A/MLX still lack owner quality acceptance. Production remains Qwen MPS/Auto/current private reference, with Interactive LaunchAgent candidate; no MLX production switch.
