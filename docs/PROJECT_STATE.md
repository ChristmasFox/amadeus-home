...
Phase 10 runs on M204 in migration-safe mode: the current immutable ARM64 image is `local/openclaw-amadeus:git-e7c3de0-20260923170152` with matching host/guest digest, merged preflight passed, container healthy, all channels disabled, no published ports, and owner delivery disabled. The previous image is retained by protected Compose backups. Phase 11 natural-prompt acceptance now passes: the raw three-question probe resolved each named alias/self in the correct order, then used local `sessions_search` and `sessions_history` only for the unregistered historical alias; no `memory_search`, `ls`, or tool failure occurred. A synthetic WebChat self lookup correctly remained `unbound` because no trusted sender metadata was supplied. Current SQLite aggregate counts are 57 session nodes, 5,791 transcript events, 5,605 active events, 1 archive row, and 2,843 transcript FTS chunks. The memory-file vector index remains dirty/empty/mismatched and was not reindexed because external embedding could expose private text; it is not required for this identity/session acceptance. The old Mac host-local LaunchAgent is still unclassified; recheck the unique-runtime gate before any owner ingress. Evidence: `.agent/checkpoints/2026-09-24-kurisu-recall-acceptance-passed.md`.
