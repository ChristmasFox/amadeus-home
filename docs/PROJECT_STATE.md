...
2026-09-24 WhatsApp relink completed on M204: channel status is `linked/healthy/connected`; Telegram remains `ready/connected`. The detached relink worker and its child login processes were stopped after successful linking, so no duplicate login session remains. Channel status still reports no real inbound or outbound activity for either channel; owner-message acceptance, final-reply correctness, and duplicate-runtime checks remain pending. Final cutover is not committed.
