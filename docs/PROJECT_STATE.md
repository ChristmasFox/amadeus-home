...
- Docker cache patch-only candidate wall 39s→3s; plugin-dist-only 1s after stable OS/glibc/npm layers. Optional direct Opus is not selected. MLX candidate HTTP 20 short requests p50 3.30s/p95 3.52s, five normal p50 5.42s; cold/real footprint peaked 18.4 GiB and macOS swap rose ~3 GiB before settling, so memory remains an explicit gate. Four real A+MLX WhatsApp voice turns: TTS 5.947/5.824/7.533/6.420s, each one media send, inbound→media 12.0–22.2s; owner confirms handset timbre/text and typed-without-voice. A fifth real voice during synthetic contention still delivered; synthetic fifth request failed closed `503 tts_busy`. Final report update, full release gates and post-release acceptance remain. Strict doctor still reports one known optional media-adapter absence; non-strict doctor exits 0.
