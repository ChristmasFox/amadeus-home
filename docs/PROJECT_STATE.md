...
- LangBot API/DB：`local/kurisu-gateway@0.1.4`、`local/pubg-stats@3.3.4`、`local/product-radar@0.6.0`、`local/organize-emby@0.2.2`、`local/macos-nas-control@0.1.6` 均 enabled；Kurisu 是唯一 Tool，legacy plugin 不再暴露自然语言 EventListener/Tool。
...
- PUBG 时间参数修复：真实 Telegram 10:05–10:06 的“昨天战绩”被 Native Agent 生成旧式 `date`/`selector` 形状，Runtime 严格校验返回 `TOOL_INPUT_INVALID`；`kurisu-gateway@0.1.4` 已加入 PUBG schema/prompt 约束和边界兼容转换，LangBot task `35` 为 `INSTALL_READY`，实际 artifact SHA-256 为 `e5600e1886e43b0df71c1aca65edc323d4534b432eb3804045e37259733308e2`。
- `0.1.4` 实际隔离插件进程 smoke：清除 URL/secret 环境变量后，legacy `selector=yesterday` 得到 `status=ok`、5 场比赛，legacy `date=2026-09-16` 得到 `status=ok`、4 条汇总记录；未修改 PUBG 数据。该证据验证网关→Runtime，不替代 Telegram/KOOK 最终送达。
...
- 部署前发现并修正两个插件 manifest 的 YAML 本地化字段缩进错误；真实 LangBot `/install/local/preview` 对两个包均返回 `code=0`。
- source `1dc5e54` 已 push；LangBot API 安装 task `14/15` 分别将 Product Radar `0.5.8`、PUBG V3 `3.3.2` 置为 `initialized`，线上 DB runtime revision 为 `25/33`。
- agent-runtime 已激活 `local/pubg-query-engine-v3:git-1dc5e54bd521`；`/healthz`、`/homehub/health`、`scripts/doctor.sh`（0 failure / 0 warning）和 smoke 通过。
- 定向验证：agent-runtime router `5/5`、PUBG plugin `16/16`、Product Radar `39/39`、typecheck、Python compile、secret scan、diff check 通过。未代发真实 Kook/Telegram 消息；根 `pnpm test` 的既有 `review-v3-2.test.ts` runner 在本次 release 前检查中卡住，未宣称全量通过。
- 回滚：插件包目录 `.backups/langbot/20260912-003227/`、`.backups/langbot/20260912-003239/`；runtime compose `/var/lib/casaos/apps/pubg-query-engine-v3/docker-compose.yml.codex-backup.20260912-003618`。
...
- source `5a213ff` 已 push；Product Radar `0.5.7` 已安装并达到 `INSTALL_READY`（LangBot task `31`），package SHA-256 为 `d889683b2adce0a500c5c7e2f42687ea9d9393d75450af419d9be0db06c6cd8d`。
- LangBot runtime config 的共享 `model_uuid` 已指向 `arthur-combo`（UUID `4d608fdb-126b-42cd-a8a5-be1349629713`），该模型 provider 为现有 `9Router`；Product Radar `/health` 返回 `status=ok`，doctor 为 0/0。
- 回滚包为 `.backups/langbot/20260911-235421/`；没有修改 Watch 数据、Product Radar Core 或 LangBot 主体。
...
provider 而失败：Ubuntu 到该入口返回 Cloudflare `1010`，插件记录
`ActionCallError` 后未声明处理，普通聊天回答了“无法后台监控”。已将该 workspace
模型改绑现有 9Router 的 `openai-chat-completions` provider；实际模型名仍为
`gpt-5.6-luna`，默认聊天模型 `arthur-combo` 未改变。LangBot 与 plugin runtime 已重载，
文本和多模态调用均返回成功，Product Radar `/health` 为 `ok`，当前 Watch 数量为 0。
回滚数据库备份为
`/DATA/AppData/langbot/backups/langbot.db.codex-product-radar-luna-20260911-224005`。
...
- 验证已通过：LangBot patch 5/5、PUBG V3 plugin 14/14、agent-runtime 130 pass / 1 skip、Python/shell syntax、secret scan、diff check；容器内 fake Telegram adapter smoke 确认调用顺序为 `send_message(Thinking...) -> edit_message_text(最终正文)`，`scripts/doctor.sh` 为 0 failure / 0 warning。
- source `941eb10` 已 push；CasaOS `ubuntu` 当前 LangBot image 为 `local/langbot-agent:941eb1089250-20260911-130202`，回滚备份为 `/var/lib/casaos/apps/langbot/docker-compose.yml.codex-backup.20260911-130206`。
- LangBot plugin API 已安装并达到 `INSTALL_READY`：PUBG V3 `3.3.1` task `12`、Product Radar `0.5.4` task `13`、Organize Emby `0.2.1` task `14`。未代发真实 Telegram/KOOK 消息，平台 inbound smoke 仍待用户触发。
