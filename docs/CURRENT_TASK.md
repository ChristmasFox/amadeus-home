...
当前优先目标为 `docs/AMADEUS_1_4_8_OPERATION_SKULD_FINAL_CUTOVER_AND_MEMORY_CONTINUITY_GOAL.md`。仓库侧 hardening、测试、release validation、commit/push 已完成；Phase 7 source freeze、Phase 8 Avalon destination preflight 和 Phase 9 OpenClaw cold-state restore 均已完成。恢复后目标 inventory 与认证快照一致：155 个 workspace 文件、4,888 个状态文件、84 个 session/JSONL、6 个 transcript；Identity/PUBG SQLite integrity 均为 `ok`，凭据 owner 为数值 `1000:1000`。六个非 Avalon staging 容器仍运行且无 Avalon bind；OpenClaw/Product Radar、生产 authority 与 owner ingress 均保持关闭。启动目的端 OpenClaw 前，必须分类当前仍运行的旧 Mac host-local `ai.openclaw.gateway` LaunchAgent 并重新核验唯一 runtime gate。Phase 9 证据见 `.agent/checkpoints/2026-09-23-openclaw-destination-restore.md`，Phase 8 证据见 `.agent/checkpoints/2026-09-23-avalon-destination-preflight.md`。

Phase 10 启动前置硬化发现旧 `scripts/openclaw-migration-safe-start.sh --plan` 会在检查 CasaOS Compose 前提前成功退出。现已改为先验证真实合并 Compose 的无端口、loopback overlay、owner delivery=false、restart=no、只读 config mount，以及目标机已加载的不可变 Git/timestamp OpenClaw 镜像；缺失或 symlink Compose 明确 fail closed。M204 当前尚无 `/var/lib/casaos/apps/openclaw` app/Compose 和 OpenClaw image，尚未启动该服务。目标 guest `/` 可用 374 GiB，target Mac `/` 可用 393 GiB；目标 Docker 当前 3.964 GB 镜像均为 active、无可回收镜像/cache。之前报告的 22 GiB 空间是控制端本机，不是 M204；没有清理或构建。聚焦回归、unique-runtime test、FAST workflow、secrets 和语法检查通过。证据见 `.agent/checkpoints/2026-09-23-migration-safe-compose-preflight.md`。下一步是在 M204 安全构建并装载 immutable image，再准备 app Compose；重启目标 runtime 前仍需解决 host-local LaunchAgent authority 分类。
