...
## 2026-09-25：默认日文在工具路径不稳定（14:43 中文，14:45 日文）

14:43:09 的 WhatsApp 语音成功 ASR，经 `identity_resolve` 原生工具后，
Agent 最终文字结构上为中文（75 个汉字、0 个假名），14:44:17 发送一条 PTT；
用户反馈这条确实仍是汉语表达。14:44:52 又收到一条语音，14:44:56
最终文字结构上为日文（31 个假名），14:45:28 发送一条 PTT；用户报告
“仍中文”发生在后一条发出**之前**，不能混淆这两条的语言验收。
...
用户确认 14:45 那条手机端确实听起来是日文，但明确补充：该轮用户**本身用日语发问**，
所以它只能证明日语输入→日语单条 PTT，不证明中文输入时仍默认日文。14:43
