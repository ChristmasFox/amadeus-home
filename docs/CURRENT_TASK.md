...
Phase 10 已在 M204 完成：ARM64 镜像 `local/openclaw-amadeus:git-9cb5474-20260923145233` 的 host/guest digest 一致；安全 Compose preflight 通过，候选容器 healthy，loopback-only、全部渠道关闭、无 published ports、owner delivery=false，9Router 探测通过（接受 200/401）。运行时 probe 返回 `MIGRATION_SAFE_CANDIDATE=passed`，候选数为 1。Phase 11 机器验收也通过：当前 inventory 的 MEMORY.md / memory/** / workspace hashes 和 155 文件、20,172,403 bytes 与认证快照一致；84 sessions/JSONL、6 transcripts 可读；6/6 SQLite integrity `ok`（含 Identity/PUBG）；health healthy。OpenClaw 启动后 state inventory 比冷恢复时多 3 个运行期文件，数据库均 integrity `ok`。只读 `doctor --json` 返回 3 条 warning（loopback onboarding、device-pair disabled、skill_workshop sender policy），不属于 Goal 明列的机器验收条件。Phase 11 还差 operator memory/persona acceptance；旧 Mac host-local LaunchAgent authority 分类和 fresh unique-runtime 检查仍是 Phase 12 前置。详细证据见 `.agent/checkpoints/2026-09-23-migration-safe-compose-preflight.md`。
