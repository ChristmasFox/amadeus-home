...
1.4.8 仓库侧 hardening、完整 `pnpm build/typecheck/test`、secrets scan、版本校验、语法和 `git diff --check` 均通过；本轮修复导出/导入/恢复清单策略不一致，并将 HMAC helper 与端到端测试纳入 Git 白名单。Phase 6 预冻结门仍未通过：现存真实密钥包使用旧清单，当前导入器拒绝其过时的临时明文策略及缺少的清理/权限/restorePolicy 元数据。`migration-readiness.sh` 的密钥包项只查归档与 sidecar 存在，不能替代 HMAC/解密/导入验证。细节见 `.agent/checkpoints/2026-09-23-openclaw-prefreeze-gate-audit.md`。未提供精确 source-freeze token；源端仍 active，尚未移动 Avalon 或切换 owner ingress。
