...
Phase 10 启动前置硬化发现旧 `scripts/openclaw-migration-safe-start.sh --plan` 会在检查 CasaOS Compose 前提前成功退出。现已先验证真实合并 Compose 的无端口、loopback overlay、owner delivery=false、restart=no、只读 config mount，以及目标机已加载的不可变 Git/timestamp OpenClaw 镜像。M204 live `--plan` 对缺失 Compose 正确 fail closed。新增安全 apply 路径：仅在给出既有 Avalon move 批准、明确 immutable image tag 且镜像已在目标加载后，才从 Git 模板原子创建缺失 Compose；非空 app dir、symlink 或不一致文件拒绝覆盖，之后必须通过完整 merged Compose 预检才会启动。`--plan` 不做持久写入。当前 M204 仍无 OpenClaw image / app Compose，尚未启动服务；guest 根盘可用 374 GiB、target Mac 可用 393 GiB，现有 3.964 GB 镜像均 active、无可回收镜像/cache，未清理或构建。聚焦回归、unique-runtime test、FAST workflow、secrets、语法和版本检查通过。证据见 `.agent/checkpoints/2026-09-23-migration-safe-compose-preflight.md`。下一步是在 M204 构建并加载 immutable image，然后执行隔离 Phase 10 safe-mode；host-local LaunchAgent 必须在 Phase 12 owner-ingress gate 前分类。
