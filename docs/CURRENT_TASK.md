...
当前优先目标为 `docs/AMADEUS_1_4_8_OPERATION_SKULD_FINAL_CUTOVER_AND_MEMORY_CONTINUITY_GOAL.md`。仓库侧 hardening、测试、release validation、commit/push 已完成；Phase 7 source freeze、Phase 8 Avalon destination preflight 和 Phase 9 OpenClaw cold-state restore 均已完成。恢复后目标 inventory 与认证快照一致：155 个 workspace 文件、4,888 个状态文件、84 个 session/JSONL、6 个 transcript；Identity/PUBG SQLite integrity 均为 `ok`，凭据 owner 为数值 `1000:1000`。六个非 Avalon staging 容器仍运行且无 Avalon bind；OpenClaw/Product Radar、生产 authority 与 owner ingress 均保持关闭。Phase 10 可在全部 owner/public ingress 与 owner delivery 关闭的 loopback migration-safe mode 下先行验证；旧 Mac host-local `ai.openclaw.gateway` LaunchAgent 必须在 Phase 12 owner-ingress gate 前完成 authority 分类并重新核验唯一 runtime gate。Phase 9 证据见 `.agent/checkpoints/2026-09-23-openclaw-destination-restore.md`，Phase 8 证据见 `.agent/checkpoints/2026-09-23-avalon-destination-preflight.md`。

Phase 10 safe Compose preflight 已在 M204 真实通过。ARM64 镜像 `local/openclaw-amadeus:git-9cb5474-20260923145233` 已在主机和 guest 加载，digest 均为 `4e7ea2d63ff7a80d98575d60060ad3825567b6fa3fc8e3c860d7b0f8db069412`。获准 apply 原子安装 canonical Compose 并通过 merged preflight；首次因 host/guest 路径错误中止。修复后容器启动，但 exit 78；只读 `doctor --json` 定位到临时 JSON overlay 写入 schema 不接受的 `ownerNotificationDeliveryEnabled` 字段。当前容器为 exited、不健康，无公开端口/渠道。源码已修复为仅通过受支持的 Compose 环境变量 `OWNER_NOTIFICATION_DELIVERY_ENABLED=false` 禁用 delivery；post-start assertion 和 unique runtime probe 也改为检查该环境变量。原始恢复配置未变。下一步提交后保留旧失败 `/run` overlay，重新生成并重跑 safe apply。M204 包级 build/typecheck 与 70 个测试、migration-safe / unique-runtime 专项测试、控制端 secrets scan、FAST workflow、语法、版本和 diff 检查通过；目标机缺少 `rg`，其 secrets 脚本告警不作通过证明。详见 `.agent/checkpoints/2026-09-23-migration-safe-compose-preflight.md`。旧 Mac LaunchAgent 仍须在 Phase 12 owner-ingress gate 前分类。
