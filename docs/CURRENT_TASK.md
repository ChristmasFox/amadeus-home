...
Phase 10 的 safe Compose preflight 已在 M204 真实通过。ARM64 镜像 `local/openclaw-amadeus:git-9cb5474-20260923145233` 已在主机和 guest 加载，镜像 digest 均为 `4e7ea2d63ff7a80d98575d60060ad3825567b6fa3fc8e3c860d7b0f8db069412`；目标端 `--plan` 在缺少 Compose 时正确 fail closed。获准 apply 已从 Git 模板原子安装 canonical Compose、生成 loopback-only config 与 Compose overlay，merged preflight 通过；随后因脚本错误地在 macOS 主机尝试进入 Linux guest 专属 `/var/lib/casaos/apps/openclaw` 路径而中止，确认容器没有创建/启动。现已在源码修复该多余 host 侧 `cd`，需提交同步后重跑 apply。M204 guest 仍没有 OpenClaw runtime；目标配置仅有刚安装的 Compose 与 migration-safe overlay，无生产 ingress/owner delivery。包级 build/typecheck 与 70 个测试、migration-safe / unique-runtime 专项测试、控制端 secrets scan、FAST workflow、语法、版本和 diff 检查通过；目标机缺少 `rg`，其 secrets 脚本告警不作通过证据。详见 `.agent/checkpoints/2026-09-23-migration-safe-compose-preflight.md`。旧 Mac LaunchAgent 仍须在 Phase 12 owner-ingress gate 前分类。
