...
当前生产 Runtime 源码提交：`f670787`（生产 Runtime image 基于 `015df8f`，包含 `38af693`、`015df8f`、`c4f2e65`）；Kurisu Gateway source 提交：`b8e93fb`（`0.1.4`）；P0–P6 已完成，P7 已部署并完成全会话自然语言切流。
...
| P7 | `DEPLOYED / BOUNDARY_HARDENED / GLOBAL_NLU_ROLLOUT_DEPLOYED / MEDIA_TOOLS_SOURCE_DEPLOYED / R05_MEDIA_ROLLBACK_VERIFIED / TELEGRAM_ROUTE_FIXED / GATEWAY_ISOLATED_PROCESS_FIXED / GATEWAY_SECRET_PATH_FIXED / PUBG_TIME_RANGE_COMPAT_FIXED / ISOLATED_PLUGIN_SMOKE_PASSED / L4_PLATFORM_PENDING` | Runtime `local/pubg-query-engine-v3:git-015df8f`、生产开关、通知/Codex/写工具、Radar central owner、四个媒体 bind mount 已部署；`kurisu-gateway@0.1.4` 与 legacy plugin 更新均 `INSTALL_READY`；Kurisu 是唯一 Tool，普通自然语言统一由 LangBot Native Agent + Kurisu 处理；首次真实 Telegram 已证明入站路由，已修复隔离插件子进程不继承 URL 与 secret 路径的问题；实际 artifact 在清除环境变量后 `kurisu.radar.list` 返回 `status=ok`，并验证 legacy PUBG `selector/date` 输入分别返回 `status=ok`、5 场/4 条结果；媒体 scan、doctor、R01/R02、HTTP/Docker smoke 通过；旧 Runtime/插件切换和当前版本恢复均在媒体挂载下通过 | 修复后 R03/R04 真实 Telegram/KOOK 入站、引用/图片/按钮/审批、群聊边界和最终送达仍待证据 |
