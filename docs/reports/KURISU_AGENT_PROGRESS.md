...
| P7 | `PARTIAL_RUNTIME_DEPLOYED / BOUNDARY_HARDENED / PLUGIN_INSTALLED / PLATFORM_READ_GRAY_VERIFIED` | Runtime immutable image 已发布；Kurisu POST secret、localhost 宿主绑定、外部 secret 权限和 live 401/授权 smoke 已核验；`local/kurisu-gateway@0.1.0` 已安装并注册；管理员 Telegram 私聊已完成真实 `kurisu.radar.list` 调用并成功回写 | session rollout、旧 EventListener single-consumer 迁移、briefing producer、写工具/通知/Codex 灰度和完整平台回滚验收仍未完成 |
