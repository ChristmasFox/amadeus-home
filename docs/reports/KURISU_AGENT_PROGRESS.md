...
| P7 | `PARTIAL_RUNTIME_DEPLOYED / BOUNDARY_HARDENED / PLUGIN_PENDING / PLATFORM_BLOCKED` | Runtime immutable image 已发布；Kurisu POST secret、localhost 宿主绑定、外部 secret 权限和 live 401/授权 smoke 已核验 | 合法 LangBot user/support-admin session、LangBot plugin secret 挂载/安装、session rollout、旧 EventListener single-consumer 迁移、briefing producer 和真实 Telegram DM 灰度仍未完成 |
