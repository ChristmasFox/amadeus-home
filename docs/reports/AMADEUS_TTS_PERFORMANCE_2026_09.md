...
**Candidate A/MLX/Auto**: original operator-owned ~46s reference, community MLX 1.7B Base 8-bit ICL, the same OpenAI-compatible service contract, one inference worker and bounded pending slot, unchanged 120s external TTS window and `ProcessType=Interactive`. The existing A/MPS source is the rollback point, not a second running engine. OpenClaw remains the single Agent runtime. **The single-engine live candidate switch succeeded** with protected A/MPS rollback; fixed HTTP short/normal timing and cold/idle memory were measured. Real WhatsApp handset/typed quality passed in the candidate; longer-running memory stability and formal release remain pending.
