...
| 确定性测试 | PASS | Domain 8、plugin 2、Product Radar 51；typecheck/build/secret scan/diff check 已通过，收尾后复跑 |
...
| S3 CasaOS 一次性切换 | PENDING | scripts/deploy-openclaw.sh --apply --build --cleanup 尚未在最终提交执行 |
| S4 真实 OpenClaw/9Router 场景 | PENDING | 待记录模型 route/version、输入、工具名/参数、事实比对、耗时和失败项 |
| Telegram 私聊查询+连续追问 | PENDING | 待新 OpenClaw 成为唯一 consumer 后以真实私聊核验最终送达 |

## 约束

本报告只记录当前 Goal，不复用旧 Kurisu 多领域验收矩阵。健康检查不能替代业务查询，
provider trace 不能替代真实 Telegram 入站；若自然入站不可获得，只记录确切外部 blocker，
不伪造通过。所有 secrets、数据库和迁移 checkpoint 位于仓库外。
