...
Use this skill when changing agent-monorepo code, docs, Docker inputs, CasaOS configuration, or OpenClaw assets.
...
3. RUNTIME covers `plugins/pubg/**`, `plugins/amadeus/**`, `packages/pubg-domain/**`, and `apps/product-radar/src/**`. Run the affected
...
4. RELEASE is explicit. Dockerfile, `.dockerignore`, package manifests, and `pnpm-lock.yaml` require a
   RELEASE build only when deployment is requested. Use `scripts/deploy-openclaw.sh`; its default is
...
5. Route OpenClaw plugin changes through the Amadeus/OpenClaw build; treat an env-only change as an
   explicit no-build recreate, never as an image build.
