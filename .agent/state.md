...
- Current acceptance: source CasaOS OpenClaw stopped, Amadeus owner ingress off, destination `OPENCLAW_PROCESS_COUNT=0`, and `DESTINATION_AUTHORITY=NO`. Cold restore matched the authenticated source evidence: 155 workspace files, 4,888 state files, 84 session/JSONL files, 6 transcripts, both required SQLite integrity checks `ok`, and all credential entries owned by numeric `1000:1000`. Protected rollback checkpoints are retained. See `.agent/checkpoints/2026-09-23-openclaw-destination-restore.md` and `.agent/checkpoints/2026-09-23-avalon-destination-preflight.md`.
- The old Mac host-local `ai.openclaw.gateway` launchd job is still active on port 18789. Its authority must be classified and the unique-runtime gate rechecked before Phase 12 owner-ingress activation. It does not block the isolated Phase 10 loopback candidate while every channel, public ingress, and owner delivery remain disabled. No production ingress or authority switch has occurred.
- Phase 10 preflight hardening is recorded in `.agent/checkpoints/2026-09-23-migration-safe-compose-preflight.md`. M204 built/loaded image `local/openclaw-amadeus:git-9cb5474-20260923145233`; host and guest digests match. Approved apply installed canonical Compose and passed preflight. After the host-path fix, the container started but exited 78; read-only `doctor --json` found the temporary config overlay injected unsupported plugin property `ownerNotificationDeliveryEnabled`. Source now removes it and relies on supported `OWNER_NOTIFICATION_DELIVERY_ENABLED=false` Compose environment; startup assertion/runtime probe check that env. Container is currently exited, not healthy; ingress/delivery/production authority remain off. Preserve the old generated `/run` overlay, regenerate a fresh one, then rerun apply. Package build/typecheck and 70 tests, migration-safe/unique-runtime tests, local secrets scan, FAST workflow, syntax/version/diff checks pass; target lacks `rg`, so target secrets-script output is not proof. Classify host LaunchAgent before Phase 12 owner-ingress gate.
