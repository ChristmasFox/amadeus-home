...
- The old Mac host-local `ai.openclaw.gateway` launchd job is still active on port 18789. Its authority must be classified and the unique-runtime gate rechecked before any destination OpenClaw startup. No production ingress or authority switch has occurred.
- Phase 10 preflight hardening is recorded in `.agent/checkpoints/2026-09-23-migration-safe-compose-preflight.md`. The migration-safe plan previously returned success before checking target CasaOS Compose. It now validates the merged no-ingress Compose and an already loaded immutable OpenClaw image; missing/symlinked Compose fails closed. Fresh M204 probes show the app directory, Compose, and OpenClaw image are absent; guest has 374 GiB free and target Mac 393 GiB free. No image was built or pruned and no OpenClaw was started. Focused regression/runtime-gate/FAST workflow/secrets checks pass. Next: build/load the immutable image on M204, provision Compose safely, then classify the host-local LaunchAgent before runtime start.
