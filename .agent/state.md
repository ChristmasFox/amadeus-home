...
- The authoritative objective is `docs/AMADEUS_1_4_8_OPERATION_SKULD_FINAL_CUTOVER_AND_MEMORY_CONTINUITY_GOAL.md`. Phase 7 source freeze is complete; Phase 8 approval is received and destination preflight is in progress. Keep consumers off until guest-side Avalon read/content gates pass.
- Current acceptance: source CasaOS OpenClaw stopped, Amadeus owner ingress off, `DESTINATION_AUTHORITY=NO`. Phase 8 host identity/sentinel/capacity/host-write and backup hash checks pass; guest VirtioFS access is blocked and `orbctl restart nyannyan` remains in `stopping`. See `.agent/checkpoints/2026-09-23-avalon-destination-preflight.md`. The host-local iMessage-only OpenClaw LaunchAgent remains active and must be classified before destination OpenClaw startup.
