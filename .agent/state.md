...
- Three cold snapshot attempts were rejected and their partial outputs cleaned. Tar-header owner reconciliation passes its fixture; the live mismatch is the `config` subtree, with workspace, SQLite, and counts matching. Hash-only per-root state digests and ephemeral path-obfuscated state-entry fingerprints are added for the next diagnostic. No source data changed. Source OpenClaw and Product Radar remain stopped; final snapshot/backup are pending.
