...
- Destination preflight: Homebrew and the clean monorepo are present. Node and pnpm are absent, Python is 3.9.6, and `/Volumes/Avalon` is absent. The canonical guest has root access but no Docker/Docker Compose and no `/DATA/AppData`, so CasaOS is not installed.
- Source `migration-readiness.sh` is currently 0 failure / 0 warning with `OPERATION_SKULD=READY`. Destination bootstrap may begin after explicit apply, but no data/secret restore, CasaOS service startup, Avalon movement, source freeze, or cutover has occurred.
- Historical `DESTINATION_MUTATED=NO` release statements remain accurate at their timestamp. Current limited host writes are user-authorized SSH authorization, terminal proxy, GitHub account-key/standard SSH configuration, and clean-monorepo clone only.
