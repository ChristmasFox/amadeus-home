...
- Current acceptance: `SOURCE_FROZEN=YES`, `SOURCE_OPENCLAW_STOPPED=YES`, `SOURCE_OWNER_INGRESS=OFF`, `DESTINATION_AUTHORITY=NO`. Final secret bundle, cold snapshot, and HomeLab backup are verified in `.agent/checkpoints/2026-09-23-openclaw-source-frozen.md`. Avalon remains mounted on the source and unmounted on M204.
