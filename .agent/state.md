...
- The authoritative objective is `docs/AMADEUS_1_4_8_OPERATION_SKULD_FINAL_CUTOVER_AND_MEMORY_CONTINUITY_GOAL.md`. Phase 7 source freeze and approved Phase 8 Avalon destination preflight are complete. Six non-Avalon staging containers returned after guest restart; keep Avalon consumers and production authority off until their restore/cutover gates pass.
- Current acceptance: source CasaOS OpenClaw stopped, Amadeus owner ingress off, `DESTINATION_AUTHORITY=NO`. M204 host/guest identity, sentinel, capacity, content, artifact hashes, and write probes pass; guest VirtioFS is readable/writable. See `.agent/checkpoints/2026-09-23-avalon-destination-preflight.md`. Phase 9 restore has not started. The host-local iMessage-only OpenClaw LaunchAgent remains active and must be classified before destination OpenClaw startup.
