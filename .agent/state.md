...
runtime/plugin 部署、平台来源修复和 Admin Identity source 配置已完成，当前等待真实
Telegram 入站复测以及 CasaOS 环境变量应用。
...
- 已新增 `TELEGRAM_ADMIN_USER_ID` / `KOOK_ADMIN_USER_ID` 启动配置，本机 `.env` 已填入真实值并保持 ignored；CasaOS runtime 尚未重启加载。
