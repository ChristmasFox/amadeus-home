...
- Cold snapshot attempts are still rejected by the aggregate manifest comparison, and the tool cleans partial outputs. Tar-header owner reconciliation passes the cross-owner fixture but does not yet resolve the live mismatch; the diagnostic now reports field names only. No source data changed. Source OpenClaw and Product Radar remain stopped; final snapshot/backup are pending.
