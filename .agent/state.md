...
- Phase 10 preflight hardening is recorded in `.agent/checkpoints/2026-09-23-migration-safe-compose-preflight.md`. `--plan` now fails closed when target CasaOS Compose is absent and validates the merged no-ingress Compose plus a loaded immutable OpenClaw image. The exact-token apply path can install the missing canonical Compose from the Git template only after confirming the immutable image is loaded; it refuses symlinks, non-empty app dirs, and conflicting files, then requires merged preflight before startup. Fresh M204 probes show app Compose and OpenClaw image are absent; guest has 374 GiB free and target Mac 393 GiB free. No image was built/pruned and no OpenClaw was started. Focused regression/runtime-gate/FAST workflow/secrets/version checks pass. Next: build/load the immutable image on M204 and run isolated Phase 10 safe mode; classify the host-local LaunchAgent before Phase 12 owner-ingress gate.
