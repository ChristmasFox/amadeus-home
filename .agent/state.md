...
- The first approved cold-snapshot attempt was rejected because local macOS extraction ownership differed from the Linux guest metadata; its partial outputs were cleaned by the tool. `openclaw_continuity.py` now verifies numeric owner metadata from the authenticated tar headers, and a cross-owner fixture passes. Source OpenClaw and Product Radar remain stopped; final snapshot/backup are still pending.
