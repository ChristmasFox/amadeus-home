...
当前状态：PUBG Telemetry 预取与 D-mail 基础能力已部署；本轮已修复 PUBG 全部用户可见时间的北京时间展示，版本 1.1.5 已完成本地验证，待 release build/apply 和 live bundle/health 验证。此前方向性结果与来源时间范围修复已完成 1.1.4 release build/apply，live Amadeus 1.1.4 已加载并通过健康、preflight、插件注册和 bundle 规则核验。既有复盘新鲜度、relative_period 06:00 解析、缓存语义、周期复盘顺序和“所有 PUBG 路由事实必须走工具”修复已完成 release build/apply；PUBG 工具默认从持久化 SQLite 缓存/更新结果取事实，上下文只解析参数，不提供数据；周期复盘默认正序、最近一局保持倒序；部署通知已改为只读取单次发布摘要，不累计历史内容；全局上下文拆分、旧 secret fallback 清理、Codex hook 修复、内部服务 proxy bypass、
...
最新已部署 PUBG 版本：提交 `04e8902`，镜像 `local/openclaw-amadeus:git-04e8902c815a-20260919064258`，恢复点
`/DATA/AppData/openclaw/backups/amadeus-openclaw-20260919064258`；未发送未经请求的真实群聊测试消息。
