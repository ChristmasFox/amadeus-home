...
状态：PARTIAL_RUNTIME_DEPLOYED / BOUNDARY_HARDENING_READY / PLUGIN_PENDING / PLATFORM_BLOCKED
...
已补齐上线前边界硬化源码：Kurisu POST 入口要求外部 secret，未配置时 fail-closed；Runtime 宿主端口模板仅绑定 `127.0.0.1`，容器间私网访问不变。该变更完成测试后再部署，当前生产插件仍未安装。
