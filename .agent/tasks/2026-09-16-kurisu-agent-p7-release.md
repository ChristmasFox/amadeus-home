...
状态：DEPLOYED / BOUNDARY_HARDENED / GLOBAL_NLU_ROLLOUT_DEPLOYED / MEDIA_TOOLS_DEPLOYED / R05_MEDIA_ROLLBACK_VERIFIED / L4_PLATFORM_PENDING
...
已完成并部署边界硬化：Kurisu POST 入口要求外部 secret，未配置时 fail-closed；Runtime 宿主端口仅绑定 `127.0.0.1`，容器间私网访问不变。
...
剩余：部署后尚无新的真实 Telegram/KOOK 入站记录，因此引用、图片、按钮/审批、必要群聊和真实 Gateway 平台身份链路未完成。Avalon 已恢复，四个媒体 bind mount 已在 live Compose/Runtime 中确认，含媒体挂载的 R05 旧版本切换与当前版本恢复已通过且未重放业务写入。通知、Codex、写工具与 Product Radar central owner 已启用，但其真实平台触发/送达仍需 L4 证据。

验收范围：管理员 Telegram 私聊与 KOOK 目标会话验证真实入站、引用、图片、按钮、审批、Codex 任务恢复和通知补发；R05 已完成，不再重复回滚；KOOK 未迁移会话保持旧权限与旧功能。不得把 P6 fake/provider 证据当作 L3/L4 通过，也不得向群聊发测试消息。
