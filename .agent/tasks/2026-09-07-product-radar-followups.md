...
- Calibrate the 0.60 perceptual threshold against labeled clothing pairs, then evaluate a local CLIP/SigLIP provider behind the existing ImageMatcher port.
- Run a real Telegram image acceptance test only after confirming the expected recipient behavior; do not send synthetic product-match notifications.
