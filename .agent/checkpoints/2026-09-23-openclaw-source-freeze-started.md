...
Four cold-snapshot attempts have failed content/manifest comparison, and each removed its partial artifact and manifest. The verifier uses numeric owners in authenticated tar headers and the cross-owner fixture passes. The latest private fingerprint diagnostic isolates 15 `config` entries whose mode as observed after macOS extraction differs; `data` and `notifications` match. The verifier now reconstructs both owner and mode from authenticated tar headers, then compares those against exact source fingerprints. It reports only root and mismatch-category counts; no paths or state values are emitted. The failed attempts changed no source data.
