...
Three cold-snapshot attempts have failed content/manifest comparison, and each removed its partial artifact and manifest. The verifier uses numeric owners in authenticated tar headers and the cross-owner fixture passes, but a real guest retry still fails `state.treeSha256`; all reported workspace, SQLite, and count metrics match. Ownership mismatch is therefore not the only cause. The manifest now records hash-only `config`, `data`, and `notifications` subtrees so the next retry can isolate the state root without exposing paths or values. The failed attempts changed no source data.
