...
Three cold-snapshot attempts have failed content/manifest comparison, and each removed its partial artifact and manifest. The verifier uses numeric owners in authenticated tar headers and the cross-owner fixture passes, but a real guest retry still fails `state.treeSha256`, specifically the `config` subtree; workspace, SQLite, and count metrics match. The manifest records hash-only `config`, `data`, and `notifications` subtrees. The next creation attempt also compares ephemeral path-obfuscated per-entry fingerprints from the stopped source and authenticated archive, reporting only root and mismatch-category counts. No paths or state values are emitted, and the failed attempts changed no source data.
