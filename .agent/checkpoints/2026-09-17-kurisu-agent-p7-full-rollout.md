...
- 已启用生产 `native_agent_global`、通知、Codex、写工具和 Product Radar central owner；媒体挂载限定为 `/Volumes/Avalon/downloads`、`media/movies`、`media/tv`、`backups/media-organizer`。
...
- R05 可恢复回滚尚未执行。执行时必须先停止新入口/通知 Worker、保留 Kurisu SQLite 和在途任务，恢复旧 compose/plugin，验证健康后再恢复当前版本；不得重放可能已成功的写操作。
